...
from config import DEEPSDF_CACL_TRAINING
...
def _build_category_index_maps(shape_ids):
	category_by_index = []
	indices_by_category = defaultdict(list)
	for shape_idx, shape in enumerate(shape_ids):
		category_id = shape["category_id"]
		category_by_index.append(category_id)
		indices_by_category[category_id].append(shape_idx)

	indices_by_category = {
		category_id: np.array(indices, dtype=np.int64)
		for category_id, indices in indices_by_category.items()
	}

	all_indices = np.arange(len(shape_ids), dtype=np.int64)
	negative_indices_by_category = {}
	for category_id, cat_indices in indices_by_category.items():
		negative_indices_by_category[category_id] = np.setdiff1d(
			all_indices,
			cat_indices,
			assume_unique=False,
		)

	return category_by_index, indices_by_category, negative_indices_by_category
...
def _sample_triplets(
	...
	unique_anchors = np.unique(batch_indices)
	...
	for anchor_idx in unique_anchors:
		category_id = category_by_index[int(anchor_idx)]
		pos_candidates = indices_by_category[category_id]
		neg_candidates = negative_indices_by_category[category_id]
		if pos_candidates.size > 1 and neg_candidates.size > 0:
			eligible_anchors.append(int(anchor_idx))

	if not eligible_anchors:
		return np.empty((0,), dtype=np.int64), np.empty((0,), dtype=np.int64), np.empty((0,), dtype=np.int64)

	effective_triplets = min(triplets_per_batch, len(eligible_anchors))
	anchors = rng.choice(np.array(eligible_anchors, dtype=np.int64), size=effective_triplets, replace=False)

	positives = np.empty((effective_triplets,), dtype=np.int64)
	negatives = np.empty((effective_triplets,), dtype=np.int64)

	for i, anchor_idx in enumerate(anchors):
		category_id = category_by_index[int(anchor_idx)]
		pos_pool = indices_by_category[category_id]
		pos_pool = pos_pool[pos_pool != anchor_idx]
		neg_pool = negative_indices_by_category[category_id]

		positives[i] = int(rng.choice(pos_pool))
		negatives[i] = int(rng.choice(neg_pool))
...
def train_autodecoder(
	data_root,
	latent_size=DEEPSDF_CACL_TRAINING["latent_size"],
	...
	triplet_lambda=DEEPSDF_CACL_TRAINING["triplet_lambda"],
	triplets_per_batch=DEEPSDF_CACL_TRAINING["triplets_per_batch"],
	device=None,
	save_dir=DEEPSDF_CACL_TRAINING["save_dir"],
):
	if device is None:
	...
	code_bound = DEEPSDF_CACL_TRAINING["code_bound"]
	latents = torch.nn.Embedding(num_shapes, latent_size, max_norm=code_bound).to(device)
	torch.nn.init.normal_(
		latents.weight,
		mean=0.0,
		std=DEEPSDF_CACL_TRAINING["code_init_stddev"] / math.sqrt(latent_size),
	)

	schedule_specs = DEEPSDF_CACL_TRAINING["lr_schedules"]
	...
	category_by_index, indices_by_category, negative_indices_by_category = _build_category_index_maps(selected_shape_ids)
	...
	samples_per_scene = batch_points or DEEPSDF_CACL_TRAINING["samples_per_scene"]
	scenes_per_batch = DEEPSDF_CACL_TRAINING["scenes_per_batch"]
	batch_split = DEEPSDF_CACL_TRAINING["batch_split"]
	clamp_dist = DEEPSDF_CACL_TRAINING["clamp_dist"]
	do_code_regularization = DEEPSDF_CACL_TRAINING["code_regularization"]
	code_reg_lambda = DEEPSDF_CACL_TRAINING["code_regularization_lambda"]
	grad_clip_norm = DEEPSDF_CACL_TRAINING["grad_clip_norm"]
	log_frequency = DEEPSDF_CACL_TRAINING["log_frequency"]
	snapshot_frequency = DEEPSDF_CACL_TRAINING["snapshot_frequency"]
	additional_snapshots = set(DEEPSDF_CACL_TRAINING["additional_snapshots"])
	...
	logger.info("Starting DeepSDF-CACL training")
	...
	logger.info(
		"CACL | triplet_lambda=%.6f triplet_margin=%.6f triplets_per_batch=%d",
		triplet_lambda,
		triplet_margin,
		triplets_per_batch,
	)
	...
	for epoch in range(1, epochs + 1):
		...
		for i in range(0, num_shapes, scenes_per_batch):
			...
			anchor_idx, positive_idx, negative_idx = _sample_triplets(
				...
				negative_indices_by_category=negative_indices_by_category,
				rng=rng,
				triplets_per_batch=triplets_per_batch,
			)

			if anchor_idx.size > 0:
				anchor_tensor = torch.from_numpy(anchor_idx).to(device)
				positive_tensor = torch.from_numpy(positive_idx).to(device)
				negative_tensor = torch.from_numpy(negative_idx).to(device)

				z_anchor = latents(anchor_tensor)
				z_positive = latents(positive_tensor)
				z_negative = latents(negative_tensor)

				triplet_loss = F.relu(
					torch.sum((z_anchor - z_positive) ** 2, dim=1)
					- torch.sum((z_anchor - z_negative) ** 2, dim=1)
					+ triplet_margin
				).mean()
			else:
				triplet_loss = torch.tensor(0.0, device=device)

			total_batch_loss = recon_loss + (triplet_lambda * triplet_loss)
			...
			total_tri += float(triplet_loss.item())
			count += 1

			if (count % log_frequency) == 0:
				logger.info(
					"Epoch %d | batch %d | running_total=%.6f running_recon=%.6f running_triplet=%.6f",
					epoch,
					count,
					total_loss / count,
					total_recon / count,
		...
		avg_tri = total_tri / max(1, count)
		logger.info(
			"Epoch %d completed | duration=%.1fs | avg_total=%.6f avg_recon=%.6f avg_triplet=%.6f",
			epoch,
			epoch_time,
			avg_loss,
			avg_recon,
		...
		if epoch % snapshot_frequency == 0 or epoch in additional_snapshots:
			torch.save(ckpt, save_path / f"deepsdf_cacl_epoch_{epoch}.pth")
			logger.info("Saved checkpoint: %s", save_path / f"deepsdf_cacl_epoch_{epoch}.pth")

		torch.save(ckpt, save_path / "deepsdf_cacl_latest.pth")
		logger.info("Updated latest checkpoint: %s", save_path / "deepsdf_cacl_latest.pth")
...
def main():
	parser = argparse.ArgumentParser(description="Train DeepSDF auto-decoder with CACL triplet loss")
	parser.add_argument(
		"data_root",
		nargs="?",
		default=str(DEEPSDF_CACL_TRAINING["data_root"]),
		help="Root directory with preprocessed SDF (per-shape sdf.npz files)",
	)
	parser.add_argument("--latent-size", type=int, default=DEEPSDF_CACL_TRAINING["latent_size"])
	...
	parser.add_argument("--triplet-lambda", type=float, default=DEEPSDF_CACL_TRAINING["triplet_lambda"])
	parser.add_argument("--triplets-per-batch", type=int, default=DEEPSDF_CACL_TRAINING["triplets_per_batch"])
	parser.add_argument("--save-dir", type=str, default=str(DEEPSDF_CACL_TRAINING["save_dir"]))
